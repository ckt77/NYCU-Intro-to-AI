...
class Ngram:
    ...
    def get_ngram(self, corpus_tokenize: List[List[str]]):
        ...
        for sentence in corpus_tokenize:
            for i in range(len(sentence) - 1):
                token1, token2 = sentence[i], sentence[i + 1]
                features[(token1, token2)] += 1
                model[token1][token2] += 1

        # # normalization
        # for token1 in model:
        #     denominator = sum(model[token1].values())
        #     for token2 in model[token1]:
        #         model[token1][token2] /= denominator
    ...
    def train_sentiment(self, df_train, df_test):
        ...
        features = self.features.most_common(feature_num)  # return a list
        ...
        for i in tqdm(range(len(df_train['review']))):
            tokenized_sentence = self.tokenize(df_train['review'][i])
            for j in range(len(tokenized_sentence) - 1):
                group = (tokenized_sentence[j], tokenized_sentence[j + 1])
                for k in range(len(features)):
                    if group == features[k][0]:
                        train_corpus_embedding[i, k] += 1
                        break

        for i in tqdm(range(len(df_test['review']))):
            tokenized_sentence = self.tokenize(df_test['review'][i])
            for j in range(len(tokenized_sentence) - 1):
                group = (tokenized_sentence[j], tokenized_sentence[j + 1])
                for k in range(len(features)):
                    if group == features[k][0]:
        ...
        precision, recall, f1, support = precision_recall_fscore_support(
            df_test['sentiment'], y_predicted, average='macro', zero_division=1)
