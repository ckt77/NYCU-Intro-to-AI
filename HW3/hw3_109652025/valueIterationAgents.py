...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(self.iterations):
            ...
            for state in states:
                if not self.mdp.isTerminal(state):
                    ...
                    tempValues[state] = max(Q_values.values())
    ...
    def computeQValueFromValues(self, state, action):
        ...
        # Begin your code
        QValue = 0
        transitionStatesAndProbabilities = self.mdp.getTransitionStatesAndProbs(
            state, action)

        for (nextState, probability) in transitionStatesAndProbabilities:
            QValue += probability * \
