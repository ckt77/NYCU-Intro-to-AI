...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        return 0.0 if not actions else max([self.getQValue(state, action) for action in actions])
        # End your code
    ...
    def computeActionFromQValues(self, state):
        ...
        return None if not actions else random.choice([action for action in actions if self.getQValue(state, action) == maxQValue])
        # End your code
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.values[(state, action)] = (1 - self.alpha) * \
            self.values[(state, action)] + self.alpha * (reward +
